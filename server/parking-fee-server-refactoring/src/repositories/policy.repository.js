 ...
 * 다목적 목록 조회 (Find All)
 */
const findAll = async (filters, sortOptions, limit, offset) => {
    let query = `SELECT p.* FROM pf_policies p`;
    ...
    Object.keys(filters).forEach(key => {
        ...
        // 1. 날짜 범위 검색
        if (['createdAtStart', 'createdAtEnd', 'updatedAtStart', 'updatedAtEnd'].includes(key)) {
            const operator = key.endsWith('Start') ? '>=' : '<=';
            const col = key.startsWith('created') ? 'created_at' : 'updated_at';
            whereClauses.push(`p.${col} ${operator} $${paramIndex}`);
            values.push(value);
            paramIndex++;
            return;
        }
 ...
 * 삭제 (Delete)
 */
const deletePolicy = async (id) => { // delete는 예약어 이슈 방지를 위해 deletePolicy 명명 권장
    const query = `DELETE FROM pf_policies WHERE id = $1 RETURNING id`;
    const { rows } = await pool.query(query, [id]);
 ...
 * 초기화 및 재설정 (Reset and Initialize)
 */
const resetAndInitialize = async (siteId, policyList, externalClient) => {
    const client = externalClient || await pool.connect();
    const shouldRelease = !externalClient;
    ...
    try {
        ...
        // 1. 기존 삭제
        await client.query(`DELETE FROM pf_policies WHERE site_id = $1`, [siteId]);
        ...
        if (policyList && policyList.length > 0) {
            ...
            const insertQuery = `
                INSERT INTO pf_policies (site_id, type, name, description, code, config, is_system)
                VALUES ${placeholders.join(', ')}
                RETURNING *
            `;
 ...
 * 블랙리스트 활성화 (트랜잭션 Update)
 */
const updateBlacklistSelection = async (siteId, targetPolicyId) => {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        ...
        await client.query(`
            UPDATE pf_policies
            SET config = jsonb_set(config, '{is_selected}', 'false'::jsonb), updated_at = NOW()
            WHERE site_id = $1 AND type = 'BLACKLIST'
        `, [siteId]);
        ...
        const { rows } = await client.query(`
            UPDATE pf_policies
            SET config = jsonb_set(config, '{is_selected}', 'true'::jsonb), updated_at = NOW()
            WHERE id = $1
            RETURNING *
        `, [targetPolicyId]);

        if (rows.length === 0) {
            const notFoundError = new Error('정책을 찾을 수 없습니다.');
            notFoundError.status = 404;
            throw notFoundError;
        }
...
module.exports = {
    ...
    findById,
    update,
    delete: deletePolicy, // export 시 이름을 delete로 매핑
    resetAndInitialize,
