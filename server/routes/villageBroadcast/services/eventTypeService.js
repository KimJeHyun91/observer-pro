...
exports.getEventTypeList = async () => {
  ...
  try {
    ...
    logger.info('villageBroadcast/eventTypeService.js, getEventTypeList, error: ', error);
    console.log('villageBroadcast/eventTypeService.js, getEventTypeList, error: ', error);
  } finally {
...
exports.modifyEventType = async ({ eventTypeId, severityId, useWarningBoard, usePopup, useEventType }) => {
  ...
  try {
    ...
    if((global.websocket) && (res) && (res.rowCount > 0)) {
      ...
      global.websocket.emit("vb_event_type-update", { eventTypeList: {'update':res.rowCount} });
    }

    return res.rowCount;

  } catch (error) {
    logger.info('villageBroadcast/eventTypeService.js, modifyEventType, error: ', error);
    console.log('villageBroadcast/eventTypeService.js, modifyEventType, error: ', error);
