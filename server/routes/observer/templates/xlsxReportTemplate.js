...
exports.xlsxReportTemplate = (workbook, sopEvent) => {

    const worksheet = workbook.addWorksheet(`SOP처리내역_${sopEvent.idx}`, {
        pageSetup: {
            printArea: 'H6:S58',
            paperSize: 9,
    ...
    // 기본 정보
    worksheet.mergeCells('H12:I12'); // 이벤트 명
    worksheet.mergeCells('H13:I13'); // 발생 일시
    worksheet.mergeCells('H14:I14'); // 발생 장소
    worksheet.mergeCells('H15:I15'); // 확인 관리자
    worksheet.mergeCells('H16:I16'); // 확인 일시
    ...
    worksheet.mergeCells('J12:S12'); // 이벤트 명
    worksheet.mergeCells('J13:S13'); // 발생 일시
    worksheet.mergeCells('J14:S14'); // 발생 장소
    worksheet.mergeCells('J15:S15'); // 확인 관리자
    worksheet.mergeCells('J16:S16'); // 확인 일시
    ...
    j12Cell.value = sopEvent.sop_name;
    j13Cell.value = sopEvent.event_occurrence_time;
    j14Cell.value = sopEvent.location;
    j15Cell.value = sopEvent.acknowledge_user;
    j16Cell.value = sopEvent.acknowledged_at;
    ...
    worksheet.mergeCells('H19:S47'); // 칸

    const h19Cell = worksheet.getCell('H19');

    h19Cell.value = sopEvent.sop_statement ? ` ${sopEvent.sop_statement}` : ' ';
    ...
    // 오탐 사유
    worksheet.mergeCells('H49:I49');

    const h39Cell = worksheet.getCell('H49');
    ...
    worksheet.mergeCells('H50:S58'); // 칸

    const h40Cell = worksheet.getCell('H50');

    h40Cell.value = sopEvent.false_alarm_type ? ` ${sopEvent.false_alarm_type}` : ' ';
