...
type DriverAddProps = {
    driverModifyConfirm: (carNumber: string, carType: string, driverName: string, phoneNumber: string) => void;
    closeModal: () => void;
    targetModifyInfo: driverInfo;
};

const DriverModify = ({ driverModifyConfirm, closeModal, targetModifyInfo }: DriverAddProps) => {
    const [carNumber, setCarNumber] = useState<string>(targetModifyInfo?.carNumber || '');
    const [carType] = useState<string>(targetModifyInfo?.type || '');
    const [driverName, setDriverName] = useState<string>(targetModifyInfo?.driverName || '');
    const [phoneNumber, setPhoneNumber] = useState<string>(targetModifyInfo?.driverPhoneNo || '');

    const modify = () => {
        driverModifyConfirm(carNumber, carType, driverName, phoneNumber);
    };
