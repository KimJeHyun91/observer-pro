...
const validationSchema: ZodType<FormSchema> = z.object({
  ip: z
    .string()
    .min(1, { message: '출입통제 IP 주소를 입력해주세요.' })
    .ip({ message: '출입통제 IP 형식이 올바르지 않습니다.' }),
  port: z
    .string()
    .min(1, {
      message: '포트를 입력해주세요.'
  ...
  dbName: z
    .string()
    .min(1, '출입통제 데이터베이스를 입력해주세요.')
    .max(30, '출입통제 데이터베이스 이름을 20자 이하로 입력해주세요.'),
  dbUser: z
    .string()
    .min(1, '출입통제 데이터베이스 ID를 입력해주세요.')
    .max(20, '출입통제 데이터베이스 ID를 20자 이하로 입력해주세요.'),
  dbPW: z
    .string()
...
export default function AccessCtlSetting() {
  const { socketService } = useSocketConnection();
  const { setting, modify, mutate, isLoading } = useSettings('출입통제 설정');
  ...
  const isSameValues = ({ ip, port, dbName, dbUser, dbPW }: {
    ip: string;
    port: string;
    dbName: string;
    dbUser: string;
  ...
  const onSubmit = async (values: FormSchema) => {
    const { ip, port, dbName, dbUser, dbPW } = values;
    setLoading(true);
    try {
      ...
      const res = await modify(`${ip},${port},${dbName},${dbUser},${dbPW}`);
      if (res.result.success) {
        setLoading(false);
        setResMessage('출입통제 설정 정보를 저장했습니다.')
        toggleModal({
          show: true,
          title: '출입통제 설정'
        });
        ...
        console.log(res);
        setLoading(false);
        setResMessage('출입통제 설정 정보 저장에 실패했습니다.')
        toggleModal({
          show: true,
          title: '출입통제 설정'
        });
      ...
      if (error instanceof AxiosError) {
        setLoading(false);
        setResMessage(error.response?.data.message || `에러로 인해 출입통제 설정에 실패했습니다.`);
      };
    };
  };
  ...
  const onRemove = async () => {
    ...
    try {
      ...
      if (res.result.success) {
        setLoading(false);
        setResMessage('출입통제 설정 정보를 삭제했습니다.')
        toggleModal({
          show: true,
          title: '출입통제 설정'
        });
        ...
        console.log(res);
        setLoading(false);
        setResMessage('출입통제 설정 정보 삭제에 실패했습니다.')
        toggleModal({
          show: true,
          title: '출입통제 설정'
        });
  ...
  useEffect(() => {
    ...
    const accessCtlSettingSocket = socketService.subscribe('cm_settings-update', (received) => {
      if (!received) return;
      console.log(received);
      if (received.settingName === '출입통제 설정') {
        mutate();
      }
    });
    return () => accessCtlSettingSocket();
  }, [socketService, mutate]);

  return (
    <section className='p-1.5'>
      <h4 className='text-[1.2rem] mb-4'>출입통제 설정</h4>
      {isLoading ? (
        <Loading loading={isLoading} />
      ) : (
        <>
          <Form
            className='w-[46.5rem] h-[22rem] border-[2px] border-[#D9DCE3] border-solid p-4'
              ...
              >
                <div className='flex gap-3 relative right-20'>
                  ...
                  <Button
                    ...
                    className={`bg-[#17A36F] text-white ${buttonStyle} relative left-[80%]`}
                    onClick={() => {
                      setConfirmMsg('설정된 출입통제 정보를 정말로 삭제하시겠습니까?');
                      toggleModalConfirm({
                        show: true,
                        title: '출입통제 설정 정보 삭제',
                        type: 'delete'
