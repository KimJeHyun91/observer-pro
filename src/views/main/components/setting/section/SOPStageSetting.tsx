...
export default function SOPStageSetting({ idx, showGuide }: Props) {
  ...
  if (isLoadingSOPStage || !SOPStageList || !Array.isArray(SOPStageList)) {
    return (
      <Loading loading={isLoadingSOPStage} />
    )
  } else if (SOPStageList.length === 0) {
    return (
      <div className='w-full min-h-[5.625rem] border-[1px] border-solid border-[#D9DCE3] bg-white p-2 mt-2'>
        ...
        >
          <div>
            <div className='flex w-full h-[2rem] border-[1px] border-[#D9DCE3] border-solid bg-white rounded-sm items-center mb-2'>
              <p className='text-[#4A4A4A] pl-3 font-semibold'>SOP 1 단계</p>
            </div>
            <div>
  ...
  } else if (SOPStageList.length > 0 && SOPList && SOPList[0].count) {
    const stage = SOPList[0].count;
    const emptyArray = Array.from({ length: stage }, (v, i) => i + 1);
    return (
      <div className={`scroll-container overflow-x-hidden overflow-y-auto ${showGuide ? 'max-h-[13.4rem]' : 'max-h-[27.4rem]'} gap-1`}>
        ...
        {addStageMode && (
          <div className='w-full min-h-[5.625rem] border-[1px] border-solid border-[#D9DCE3] bg-white p-2 mt-2'>
            ...
            >
              <div>
                <div className='flex w-full h-[2rem] border-[1px] border-[#D9DCE3] border-solid bg-white rounded-sm justify-between items-center mb-2'>
