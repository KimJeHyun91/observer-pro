...
export default function WaterlevelThresholdSettings() {
	...
	const {
		...
		settingThresholdToWaterlevelGauge,
	...
	const handleThresholdSave = async (water_level_idx: number) => {
		try {
		  const success = await settingThresholdToWaterlevelGauge({
			 water_level_idx,
					...
					<table className="w-full">
						...
						<tbody>
							...
							{waterlevelGaugeList.map((waterlevel) => (
								<React.Fragment key={waterlevel.water_level_idx}>
									<tr className="border-t">
										<td className="p-3 text-sm">{waterlevel.water_level_name}</td>
										<td className="p-3 text-sm">{waterlevel.water_level_location}</td>
										<td className="p-3 text-sm whitespace-pre-line">
													{waterlevel.water_level_idx ?
														waterlevelAutoControlList
															...
															.map(gate => gate.outside_name)
															.join(',\n')
														: '-'
													}</td>
										<td className="p-3 text-sm">{waterlevel.curr_water_level}m</td>
										<td className="p-3 text-sm">{waterlevel.threshold}m</td>
										<td className="p-3 text-sm">
											<Button
												...
												onClick={() => {
														const status = waterlevel.use_status;
														if (status) {
															setSelectedWaterlevel(selectedWaterlevel === waterlevel.water_level_idx ? null : waterlevel.water_level_idx)
														} else {
															setAlertOpen(true);
															setErrorMessage("수위계 사용여부를 확인하세요.");
														}
													}
												}
											>
									...
									{selectedWaterlevel === waterlevel.water_level_idx && (
										<tr className="bg-gray-50">
											<td colSpan={6} className="p-0">
												<div className="bg-gray-100 p-6 relative">
													...
													<div className="mb-3 text-red-500 text-sm">
														* 설정 수위보다 현재 수위가 높아질 경우, 스피커 안내방송 주 대상된 차단기가 자동으로 닫힙니다.
													</div>
													<div className="space-y-4 mb-6 ">
														<div className="flex justify-center items-center gap-2">
															...
															<Input
																type="number"
																value={waterlevel.curr_water_level}
																disabled
														...
														<div className="flex justify-center items-center gap-2">
															...
															<Input
																type="number"
																placeholder={waterlevel.threshold}
													...
													<div className="flex justify-end">
														<Button
															...
															size="sm"
															variant="solid"
															onClick={() => handleThresholdSave(waterlevel.water_level_idx)}
														>
															임계치 저장
														</Button>
													</div>
