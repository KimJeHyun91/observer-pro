...
interface BroadcastVmsListProps {
...
const BroadcastVmsList = ({ handleEdit, setIsEditMode, selectedRows, setSelectedRows }: BroadcastVmsListProps) => {
    const [editingRow, setEditingRow] = useState<number | null>(null)
    const { control, handleSubmit, setValue } = useForm()

    const { isLoading, error, data: vmsList, mutate } = useObVms('broadcast')
    ...
    const removeVms = async () => {
        try {
            ...
            const res = await apiDeleteObVms<ApiResultStatusWrapper>(selectedNames, 'broadcast');
            if (res) {
...
export default BroadcastVmsList
